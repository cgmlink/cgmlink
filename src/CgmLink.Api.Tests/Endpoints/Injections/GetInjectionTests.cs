...
using CgmLink.Api.Endpoints.Injections.GetInjection;
...
namespace CgmLink.Api.Tests.Endpoints.Injections;
...
public class GetInjectionTests
{
    ...
    public async Task HandleAsync_Should_Return_Ok_When_Injection_Found()
    {
        ...
        var result = await Endpoint.HandleAsync(injectionId, _currentUserMock.Object,
            _injectionsRepositoryMock.Object, CancellationToken.None);
    ...
    public void HandleAsync_Should_Throw_NotFoundException_When_Injection_Not_Found()
    {
        ...
        Assert.ThrowsAsync<NotFoundException>(async () =>
            await Endpoint.HandleAsync(injectionId, _currentUserMock.Object, _injectionsRepositoryMock.Object,
                CancellationToken.None));
    }

    [Test]
    public void HandleAsync_Should_Return_Unauthorized_When_User_Not_Authenticated()
    {
        _currentUserMock.Setup(c => c.GetUserId()).Throws<UnauthorizedAccessException>();

        Assert.That(
            () => Endpoint.HandleAsync(Guid.NewGuid(), _currentUserMock.Object, _injectionsRepositoryMock.Object,
                CancellationToken.None),
