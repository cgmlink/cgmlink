...
using Endpoint = CgmLink.Nutrition.Endpoints.SearchProduct.Endpoint;
using GPRepository = CgmLink.Data.Repository;

namespace CgmLink.Nutrition.Tests.Endpoints.SearchProduct;
...
public class SearchProductEndpointTests
{
    ...
    private static IEnumerable<Ingredient> GenerateIngredients(Guid userId, int count = 10)
    {
        for (var i = 0; i < count; i++)
        {
            yield return new Ingredient { Id = Guid.NewGuid(), UserId = userId, Barcode = $"{i}", Created = DateTimeOffset.Now, Name = $"Ingredient {i}", Uom = CgmLink.Data.Enums.UnitOfMeasurement.Grams };
        }
    }
}